...
export const LiveConferenceDashboard = ({ incidentId }: LiveConferenceDashboardProps) => {
      ...
      <Card className="w-full">
        <CardContent className="pt-6">
          <div className="flex items-center justify-center">
            ...
            <span className="ml-3 text-sm text-gray-500">Connecting to emergency conference...</span>
          </div>
        </CardContent>
      </Card>
    );
  ...
  if (error) {
    return (
      <Card className="w-full border-yellow-200 bg-yellow-50">
        <CardContent className="pt-6">
          <p className="text-sm text-yellow-800">Unable to load conference status: {error}</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const getStatusBadge = (status: string) => {
    const badges = {
      calling: <Badge variant="outline" className="bg-blue-50">📞 Calling</Badge>,
      ringing: <Badge variant="outline" className="bg-yellow-50">🔔 Ringing</Badge>,
      in_conference: <Badge className="bg-green-500">✅ Connected</Badge>,
      left: <Badge variant="secondary">👋 Left</Badge>,
      ...
      <Card className="border-red-200 bg-red-50">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Live Emergency Conference
          </CardTitle>
          <CardDescription>
            {conference.status === 'active' ? (
              <span className="text-green-600 font-medium">🟢 Conference Active</span>
            ) : (
              <span className="text-gray-600">Conference Ended</span>
            )}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="flex flex-col">
              <span className="text-xs text-gray-500">Total Participants</span>
              <span className="text-2xl font-bold">{conference.total_participants}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-xs text-gray-500">Currently Connected</span>
            ...
            <div className="flex flex-col">
              ...
              <span className="text-sm font-medium mt-1">
                {userInConference ? (
                  <Badge className="bg-green-500">🟢 On Call</Badge>
                ) : (
            ...
            <div className="flex flex-col">
              <span className="text-xs text-gray-500">Responders</span>
          ...
          {conference.started_at && (
            <div className="mt-4 flex items-center gap-2 text-sm text-gray-600">
              ...
              <span>
                Started {formatDistanceToNow(new Date(conference.started_at), { addSuffix: true })}
              </span>
            </div>
          )}
            ...
            {participants.map((participant) => (
              ...
              >
                  ...
                  <div>
                    ...
                    {participant.eta_minutes && (
                      <p className="text-xs text-blue-600 mt-1">
                        ETA: {participant.eta_minutes} minutes
                      </p>
                    )}
                ...
                <div className="flex items-center gap-2">
                  {participant.muted && <Badge variant="secondary">🔇 Muted</Badge>}
                  {participant.hold && <Badge variant="secondary">⏸️ Hold</Badge>}
