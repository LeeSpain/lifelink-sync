...
serve(async (req) => {
  ...
  try {
    ...
    // Remove associated permissions if connection was active and in family circle
    if (connection.status === 'active' && connection.contact_user_id && connection.type === 'family_circle') {
    ...
    console.error('Error in connections-revoke:', error)
    return new Response(
