...
-- Add unique constraint on slug for published blog content
CREATE UNIQUE INDEX CONCURRENTLY idx_marketing_content_slug_unique 
ON public.marketing_content (slug) 
WHERE platform = 'blog' AND status = 'published';

-- Add index for better blog content queries
CREATE INDEX CONCURRENTLY idx_marketing_content_blog_seo 
