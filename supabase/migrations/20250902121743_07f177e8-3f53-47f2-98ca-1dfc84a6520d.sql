...
-- 6. Fix communication_metrics_summary table security (if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_catalog.pg_tables WHERE schemaname = 'public' AND tablename = 'communication_metrics_summary') THEN
    EXECUTE 'ALTER TABLE public.communication_metrics_summary ENABLE ROW LEVEL SECURITY';
    EXECUTE 'DROP POLICY IF EXISTS "admins_only_communication_metrics" ON public.communication_metrics_summary';
    EXECUTE 'CREATE POLICY "admins_only_communication_metrics" ON public.communication_metrics_summary FOR ALL USING (public.is_admin())';
  END IF;
END$$;