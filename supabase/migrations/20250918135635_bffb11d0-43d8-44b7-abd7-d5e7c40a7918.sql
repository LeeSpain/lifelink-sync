...
-- Ensure email_templates has all needed columns (table may pre-exist with different schema)
...
-- Make body_template nullable since new records may use html_template instead
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'email_templates' AND column_name = 'body_template' AND is_nullable = 'NO') THEN
    ALTER TABLE email_templates ALTER COLUMN body_template DROP NOT NULL;
  END IF;
END$$;

-- Create email_templates table for reusable templates (if not exists)
...
-- Ensure email_automation_settings has all needed columns
ALTER TABLE IF EXISTS email_automation_settings
ADD COLUMN IF NOT EXISTS template_id UUID,
...
-- Insert default email templates (skip if name already exists)
DO $$
BEGIN
...
-- Insert default automation settings (skip on any error - table schema may differ)
DO $$
BEGIN
